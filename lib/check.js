...
/**
 * When a checked URL doesn't behave like a paid resource, it's often because
 * the caller pointed at the wrong endpoint of a service that has several
 * (docs, health, discovery...) and only one of them actually charges. Many
 * x402 sellers publish a `.well-known/x402` manifest (RFC 8615-style) listing
 * their real payable endpoints - if one exists, surface it as a suggestion
 * instead of leaving the caller with just "this doesn't work".
 */
async function suggestPaidEndpoints(url) {
  let origin;
  try {
  ...
  try {
    const res = await fetch(`${origin}/.well-known/x402`, { signal: AbortSignal.timeout(TIMEOUT_MS) });
    if (!res.ok) return null;
    const data = await res.json();
    const endpoints = Array.isArray(data.endpoints) ? data.endpoints : [];
    const urls = endpoints
      .map(e => (e && typeof e.url === "string" ? e.url : null))
      .filter(Boolean)
      .filter(u => u !== url);
    if (urls.length === 0) return null;
    return { manifestUrl: `${origin}/.well-known/x402`, paidEndpoints: urls };
  } catch {
    return null;
  }
}
...
export async function checkX402Endpoint(url) {
  ...
  // The URL doesn't look payable - check whether the site publishes a
  // .well-known/x402 manifest pointing at its actual paid endpoint(s), so a
  // wrong-URL mistake reads as "try this instead" rather than "broken".
  const suggestion = response.status !== 402 ? await suggestPaidEndpoints(url) : null;
